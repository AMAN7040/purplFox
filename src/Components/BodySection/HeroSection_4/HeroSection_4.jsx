...
const HeroSection_4 = ({ interval = 3000 }) => {
  ...
  const imageClass = styles[`img_${currentIndex}`] || "";
          ...
          <div className="relative h-full w-full">
            <div className="absolute top-1/2 left-1/2 h-full w-full -translate-x-1/2 -translate-y-1/2">
              <img
                ...
                className={`${imageClass} -z-1 mx-auto h-full w-full object-cover transition-transform duration-300 ease-in-out`}
              />
            </div>

            <div
              className={`${currentIndex === 0 ? "flex" : "hidden"} flex-center relative z-2 mx-auto my-0 h-full w-full overflow-hidden px-[calc(var(--spacing)*2.5)] py-[calc(var(--spacing)*4)] text-center font-secondary md:px-[calc(var(--spacing)*4)]`}
            >
              <h2 className="m-auto max-w-[800px] text-lg font-light break-words text-orange-400 sm:text-xl md:text-3xl lg:text-5xl">
                VersaFlex Multi-Purpose Shorts
              </h2>
            </div>
          </div>
        </div>
        {/* Buttons */}
        <div className="absolute bottom-[50px] z-3 mx-auto my-0 flex w-full justify-between gap-[12px] px-[calc(var(--spacing)*2.5)] sm:justify-center sm:gap-[16px] lg:px-[calc(var(--spacing)*4)]">
          {/* Previous Button */}
          <button
            onClick={() => {
              prevSlide();
